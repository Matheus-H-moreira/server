...
if (botaoConcluir) {
    botaoConcluir.addEventListener('click', async (event) => {
        ...
        try {
            const verificaEmail = await fetch(`https://tiaw-obesidade.onrender.com/usuarios?email=${encodeURIComponent(email)}`);
        ...
        try {
            const response = await fetch('https://tiaw-obesidade.onrender.com/usuarios', {
                method: 'POST',
                headers: {
...
document.getElementById('edit-profile-form')?.addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch(`https://tiaw-obesidade.onrender.com/usuarios/${usuarioLogado.id}`, {
            method: 'PUT',
